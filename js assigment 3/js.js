...
returnTitle(products)

// without function
let sum = []
for(let i = 0; i<products.length; i++){

sum.push(products[i].title)
}
console.log(sum)
...
getUniqueColor(products)
console.log(getUniqueColor(products))
// Exercise 3
// Get the total quantity of all products.
// Example: 23
// Try with function or without function
function sumNumber(products){
  let hn = 0;
  for(let i =0 ; i < products.length; i++){
  
  for(let j =0 ; j < products[i].variations.length; j++){
      hn += products[i].variations[j].quantity
...
if(products[i].reviews[j].status === true){
  ...
  // console.log(products[i].reviews[j].status)
  putReviews.push(products[i].reviews)
}
}
}
return putReviews
}
 getTrueReviews (products);
...
if(maxVariation === null || products[i].variations[j].price > maxVariation.price){
maxVariation = products[i].variations[j]
}
}
}
return maxVariation
...
average += products[i].reviews[j].rating;
...
function getId(products, productId){
...
console.log(getId(products, 105))
